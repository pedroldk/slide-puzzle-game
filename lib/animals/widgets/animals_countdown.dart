...
/// {@template animals_countdown}
/// Displays the countdown before the puzzle is started.
/// {@endtemplate}
class AnimalsCountdown extends StatefulWidget {
  /// {@macro animals_countdown}
  const AnimalsCountdown({
    Key? key,
  ...
  @override
  State<AnimalsCountdown> createState() => _AnimalsCountdownState();
}

class _AnimalsCountdownState extends State<AnimalsCountdown> {
  ...
  Widget build(BuildContext context) {
    return AudioControlListener(
      ...
      child: BlocListener<AnimalsPuzzleBloc, AnimalsPuzzleState>(
        listener: (context, state) {
          ...
          if (state.status == AnimalsPuzzleStatus.started) {
          ...
          large: (_, __) => BlocBuilder<AnimalsPuzzleBloc, AnimalsPuzzleState>(
            builder: (context, state) {
              ...
              if (state.secondsToBegin > 0) {
                return AnimalsCountdownSecondsToBegin(
                  key: ValueKey(state.secondsToBegin),
                  secondsToBegin: state.secondsToBegin,
                );
              } else {
                return const AnimalsCountdownGo();
              }
            },
          ),
        ),
...
/// {@template animals_countdown_seconds_to_begin}
/// Display how many seconds are left to begin the puzzle.
/// {@endtemplate}
@visibleForTesting
class AnimalsCountdownSecondsToBegin extends StatefulWidget {
  /// {@macro animals_countdown_seconds_to_begin}
  const AnimalsCountdownSecondsToBegin({
    Key? key,
  ...
  @override
  State<AnimalsCountdownSecondsToBegin> createState() =>
      _AnimalsCountdownSecondsToBeginState();
}

class _AnimalsCountdownSecondsToBeginState
    extends State<AnimalsCountdownSecondsToBegin>
...
/// {@template animals_countdown_go}
/// Displays a "Go!" text when the countdown reaches 0 seconds.
/// {@endtemplate}
@visibleForTesting
class AnimalsCountdownGo extends StatefulWidget {
  /// {@macro animals_countdown_go}
  const AnimalsCountdownGo({Key? key}) : super(key: key);

  @override
  State<AnimalsCountdownGo> createState() => _AnimalsCountdownGoState();
}

class _AnimalsCountdownGoState extends State<AnimalsCountdownGo>
