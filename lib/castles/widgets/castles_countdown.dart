...
/// {@template castles_countdown}
/// Displays the countdown before the puzzle is started.
/// {@endtemplate}
class CastlesCountdown extends StatefulWidget {
  /// {@macro castles_countdown}
  const CastlesCountdown({
    Key? key,
  ...
  @override
  State<CastlesCountdown> createState() => _CastlesCountdownState();
}

class _CastlesCountdownState extends State<CastlesCountdown> {
  ...
  Widget build(BuildContext context) {
    return AudioControlListener(
      ...
      child: BlocListener<CastlesPuzzleBloc, CastlesPuzzleState>(
        listener: (context, state) {
          ...
          if (state.status == CastlesPuzzleStatus.started) {
        ...
        child: ResponsiveLayoutBuilder(
          ...
          large: (_, __) => BlocBuilder<CastlesPuzzleBloc, CastlesPuzzleState>(
            builder: (context, state) {
              ...
              if (state.secondsToBegin > 0) {
                return CastlesCountdownSecondsToBegin(
                  key: ValueKey(state.secondsToBegin),
                  secondsToBegin: state.secondsToBegin,
                );
              } else {
                return const CastlesCountdownGo();
              }
            },
          ),
        ),
...
/// {@template castles_countdown_seconds_to_begin}
/// Display how many seconds are left to begin the puzzle.
/// {@endtemplate}
@visibleForTesting
class CastlesCountdownSecondsToBegin extends StatefulWidget {
  /// {@macro castles_countdown_seconds_to_begin}
  const CastlesCountdownSecondsToBegin({
    Key? key,
  ...
  @override
  State<CastlesCountdownSecondsToBegin> createState() =>
      _CastlesCountdownSecondsToBeginState();
}

class _CastlesCountdownSecondsToBeginState
    extends State<CastlesCountdownSecondsToBegin>
...
/// {@template castles_countdown_go}
/// Displays a "Go!" text when the countdown reaches 0 seconds.
/// {@endtemplate}
@visibleForTesting
class CastlesCountdownGo extends StatefulWidget {
  /// {@macro castles_countdown_go}
  const CastlesCountdownGo({Key? key}) : super(key: key);

  @override
  State<CastlesCountdownGo> createState() => _CastlesCountdownGoState();
}

class _CastlesCountdownGoState extends State<CastlesCountdownGo>
