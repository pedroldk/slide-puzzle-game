...
/// {@template dinosaurs_puzzle_tile}
/// Displays the puzzle tile associated with [tile]
/// based on the puzzle [state].
/// {@endtemplate}
class DinosaursPuzzleTile extends StatefulWidget {
  /// {@macro dinosaurs_puzzle_tile}
  const DinosaursPuzzleTile({
    Key? key,
    required this.tile,
    required this.state,
  ...
  @override
  State<DinosaursPuzzleTile> createState() => DinosaursPuzzleTileState();
}

/// The state of [DinosaursPuzzleTile].
@visibleForTesting
class DinosaursPuzzleTileState extends State<DinosaursPuzzleTile>
  ...
  Widget build(BuildContext context) {
    ...
    final hasStarted = status == DinosaursPuzzleStatus.started;
    final puzzleIncomplete =
        context.select((PuzzleBloc bloc) => bloc.state.puzzleStatus) ==
            PuzzleStatus.incomplete;

    final movementDuration = status == DinosaursPuzzleStatus.loading
      ...
      child: AnimatedAlign(
        ...
        child: ResponsiveLayoutBuilder(
          small: (_, child) => SizedBox.square(
            key: Key('dinosaurs_puzzle_tile_small_${widget.tile.value}'),
            dimension: _TileSize.small,
            child: child,
          ),
          medium: (_, child) => SizedBox.square(
            key: Key('dinosaurs_puzzle_tile_medium_${widget.tile.value}'),
            dimension: _TileSize.medium,
            child: child,
          ),
          large: (_, child) => SizedBox.square(
            key: Key('dinosaurs_puzzle_tile_large_${widget.tile.value}'),
          ...
          child: (_) => MouseRegion(
            ...
            child: ScaleTransition(
              key: Key('dinosaurs_puzzle_tile_scale_${widget.tile.value}'),
              scale: _scale,
              child: IconButton(
